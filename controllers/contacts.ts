import type { HandlerFunc, Context } from 'https://deno.land/x/abc@v1.2.2/mod.ts';
...
export const createContact: HandlerFunc = async (c: Context) => {
	try {
		if (c.request.headers.get('content-type') !== 'application/json') throw new ErrorHandler('Invalid body', 422);
		...
		if (!Object.keys(body).length) throw new ErrorHandler('Request body can not be empty!', 400);
...
export const getAllContact: HandlerFunc = async (c: Context) => {
	try {
		const getContacts = await contacts.find() as Contact[];
		if (getContacts) {
...
export const getOneContact: HandlerFunc = async (c: Context) => {
	try {
		...
		const getContact = await contacts.findOne({ _id: { $oid: id }}) as Contact;
		if (getContact) {
...
export const updateContact: HandlerFunc = async (c: Context) => {
	try {
		...
		if (getContact) {
			...
			if (matchedCount) return c.string('Contact updated successfully!', 204);
...
export const deleteContact: HandlerFunc = async (c: Context) => {
	try {
		...
		if (getContact) {
			...
			if (deleteCount) return c.string('Contact deleted successfully!', 204);
